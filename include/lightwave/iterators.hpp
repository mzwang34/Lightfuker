...
/**
 * @brief Iterates up to a given number in geometrically increasing increments,
 * e.g., {[0,1), [1,3), [3,7), [7,15), …} up to the requested number.
 * This is particularly useful when rendering images progressively to chunk the
 * numbers of samples per pixel, so that early on frequent updates are performed
 * and fewer updates later on.
 */
class GeometricallyChunkedRange {
public:
    struct iterator {
        Range operator*() const { return { m_start, end() }; }
        bool operator!=(const iterator &other) const {
            return m_start != other.m_start;
        }
        iterator &operator++() {
    ...
    private:
        friend class GeometricallyChunkedRange;
        iterator(int start, int end, int maxChunkSize)
            : m_start(start), m_end(end), m_maxChunkSize(maxChunkSize) {}
        int m_start, m_end;
        int m_maxChunkSize;

        int end() const {
            const int currentChunkSize = std::min(m_start + 1, m_maxChunkSize);
            return std::min(m_start + currentChunkSize, m_end);
        }
    };

    /// @brief Constructs a range [0, count), which is iterated in geometrical
    /// increments.
    GeometricallyChunkedRange(int count, int maxChunkSize)
        : m_count(count), m_maxChunkSize(maxChunkSize) {}

    iterator begin() const { return iterator(0, m_count, m_maxChunkSize); }
    iterator end() const { return iterator(m_count, m_count, 0); }

private:
    int m_count, m_maxChunkSize;
};

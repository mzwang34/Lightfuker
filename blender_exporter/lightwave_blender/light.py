...
def _export_area_light(registry: SceneRegistry, instance_node, light: bpy.types.Light, inst: bpy.types.DepsgraphObjectInstance):
    ...
    # Disable area normalization if requested (Blender 4.5+)
    ...
    if bpy.app.version < (4, 0, 0):
        ...
        # This is (ignoring spread) the following (see https://projects.blender.org/blender/blender/commit/482211b7e0 and https://projects.blender.org/blender/blender/issues/113117):
        return 1 / (3.14159 * 4 * (lensqr_x * lensqr_y) ** 0.5)
...
def export_light(registry: SceneRegistry, inst):
    ...
    has_temperature = getattr(light, 'use_temperature', False)
    if has_temperature:
        temperature_args = { 'temperature': light.temperature }
    else:
        temperature_args = {}

    exposure_scale=2**getattr(light, 'exposure', 0.0)

    if light.type == "POINT" and light.shadow_soft_size < 1e-3:
        power = str_flat_array([ exposure_scale * light.energy * light.color[chan] for chan in range(3) ])
        position = str_flat_array([ inst.matrix_world[dim][3] for dim in range(3) ])
        return [XMLNode("light", type="point", position=position, power=power, **temperature_args)]

    if light.type == "SUN":
        intensity = str_flat_array([ exposure_scale * light.energy * light.color[chan] for chan in range(3) ])
    ...
    emission = [
        exposure_scale * normalization * light.energy * light.color[chan]
        for chan in range(3)
    ]

    if has_temperature:
        instance_node.add("emission", type="lambertian").add(
            "texture", name="emission", type="blackbody", temperature=light.temperature, tint=str_flat_array(emission))
    else:
        instance_node.add("emission", type="lambertian").add(

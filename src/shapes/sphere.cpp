...
public:
    ...
    AreaSample sampleArea(const Point &origin, Sampler &rng) const override {
        ...
        // float sin_theta = std::sqrt(std::max(0.f, 1.f - cos_theta *
        // cos_theta)); float phi = 2.f * Pi * v;
        float sin_theta_2 = 1.f - cos_theta * cos_theta;

        float cos_alpha =
            sin_theta_2 / sin_theta_max +
            cos_theta *
                std::sqrt(1 - sin_theta_2 / (sin_theta_max * sin_theta_max));
        ...
        sample.pdf =
            Inv2Pi / (1.f - cos_theta_max) * cosTheta / dist2; // area pdf

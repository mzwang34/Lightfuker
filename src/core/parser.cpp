...
struct SceneParser::PrimitiveNode : public SceneParser::Node {
    ...
    void attribute(const std::string &attrKey,
        ...
        if (attrKey == "name") {
            ...
            lightwave_throw(
                "unsupported attribute \"%s\" on <%s />", attrKey, tag);
        }
    }
...
struct SceneParser::DefaultNode : public SceneParser::Node {
    std::string variable;
    std::string value;

    DefaultNode(const ref<Node> &parent) : Node(parent) {}

    void attribute(const std::string &attrKey,
                   const std::string &attrValue) override {
        if (attrKey == "name") {
            this->variable = attrValue;
        } else if (attrKey == "value") {
            this->value = attrValue;
        } else {
            lightwave_throw(
    ...
    void close() override {
        if (sceneVariables.find(variable) == sceneVariables.end()) {
...
struct SceneParser::TransformNode : public SceneParser::Node {
    ...
    void attribute(const std::string &attrKey,
        ...
        if (tag == "matrix") {
            if (attrKey == "value") {
                this->matrix = parse_string<Matrix4x4>(attrValue);
                return;
            }
        }
        ...
        if (tag == "translate" || tag == "scale") {
            if (attrKey == "x") {
                this->value.x() = parse_string<float>(attrValue);
                return;
            }
            if (attrKey == "y") {
                this->value.y() = parse_string<float>(attrValue);
                return;
            }
            if (attrKey == "z") {
                this->value.z() = parse_string<float>(attrValue);
                return;
            }
            if (attrKey == "value") {
                this->value = parse_string<Vector>(attrValue);
                return;
            }
        }
        ...
        if (tag == "rotate") {
            if (attrKey == "axis") {
                this->axis = parse_string<Vector>(attrValue);
                return;
            }
            if (attrKey == "angle") {
                this->angle = parse_string<float>(attrValue);
                return;
            }
        }
        ...
        if (tag == "lookat") {
            if (attrKey == "origin") {
                this->origin = parse_string<Vector>(attrValue);
                return;
            }
            if (attrKey == "target") {
                this->target = parse_string<Vector>(attrValue);
                return;
            }
            if (attrKey == "up") {
                this->up = parse_string<Vector>(attrValue);
                return;
            }
        }

        lightwave_throw("unsupported attribute \"%s\" on <%s />", attrKey, tag);
    }
...
void SceneParser::open(const std::string &tag,
    ...
    if (tag == "lightwave") {
        ...
        m_stack.push(std::make_shared<DefaultNode>(parent));
    } else if (tag == "ref") {
...
SceneParser::SceneParser(const std::filesystem::path &path)
    : m_progress("parsing") {
    logger(EInfo, "loading scene from %s", path);
